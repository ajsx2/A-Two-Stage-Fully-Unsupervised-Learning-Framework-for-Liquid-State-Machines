...
class SpikingSOMLayer:
    ...
    def __init__(self, n_input: int, n_competitive: int = 400, device: str = 'cpu'):
        ...
        self.sigma_init = n_competitive * 0.2
...
class EnhancedHybridSpikeEncoder:
    ...
    def __init__(self, time_steps: int = 50, device: str = 'cuda'):
        ...
        self.device = device
        self.ttfs_window = (0, 20)
        self.poisson_window = (20, 50)
        
        self.high_intensity_threshold = 0.5
        self.edge_threshold = 0.12
    
    def detect_edges(self, img: torch.Tensor, threshold: float = 0.12) -> torch.Tensor:
...
class EnhancedExcitatoryInhibitorySTDP:
    ...
    def __init__(self, n_neurons: int, device: str = 'cpu', excitatory_ratio: float = 0.8):
        ...
        self.a_plus_exc_fast = 0.015
        self.a_minus_exc_fast = 0.010
        self.tau_plus_exc_fast = 20.0
        self.tau_minus_exc_fast = 20.0
        
        self.a_plus_exc_slow = 0.005
        self.a_minus_exc_slow = 0.003
        self.tau_plus_exc_slow = 40.0
        self.tau_minus_exc_slow = 40.0
        
        self.a_plus_inh = 0.008
        self.a_minus_inh = 0.020
        self.tau_plus_inh = 22.0
        self.tau_minus_inh = 22.0
...
class OptimizedTemporalLiquidLayer:
    ...
    def __init__(self, n_input: int, n_neurons: int = 450, device: str = 'cpu',
        ...
        self.stdp = EnhancedExcitatoryInhibitorySTDP(n_neurons, device, excitatory_ratio=0.8)
        
        self.w_input = torch.randn(n_input, n_neurons, device=device) * 0.25
        mask = torch.rand_like(self.w_input) < 0.45
        self.w_input = self.w_input * mask.float()
        
        self.w_recurrent = torch.randn(n_neurons, n_neurons, device=device) * 0.12
        mask = torch.rand_like(self.w_recurrent) < 0.18
    ...
    def process(self, input_spikes: torch.Tensor, time_steps: int,
        ...
        for t in range(time_steps):
            ...
            if apply_stdp and training and t % 4 == 0 and t > 8:
                self.w_recurrent = self.stdp.compute_weight_update(
                    self.w_recurrent, spike, spike, lr_scale=0.08
                )
                self._apply_ei_constraints()
...
class ImprovedStableSOM:
    ...
    def train_unsupervised(self, features: torch.Tensor, n_epochs: int = 250,
        ...
        for epoch in pbar:
            ...
            current_lr = learning_rate * np.exp(-2.2 * progress)
            
            sigma_init = 4.0
            sigma_final = 0.35
            current_sigma = sigma_final + (sigma_init - sigma_final) * np.exp(-3.5 * progress)
            ...
            if visualizer is not None:
                ...
                unique_bmu = len(torch.unique(bmu_test))
                activation_rate = unique_bmu / self.n_neurons
                
                feature_variance = features.var().item()
                
                visualizer.record_training_step(
                    epoch, avg_error, activation_rate, 
                    feature_variance, current_lr
                )
            
            pbar.set_postfix({
                'Error': f"{avg_error:.4f}",
                'Best': f"{self.best_error:.4f}",
                'LR': f"{current_lr:.3f}",
    ...
    def assign_labels(self, features: torch.Tensor, labels: torch.Tensor):
        ...
        neuron_votes = {i: {} for i in range(self.n_neurons)}
        
        batch_size = 256
        n_batches = (features.shape[0] + batch_size - 1) // batch_size
        
        for batch_idx in tqdm(range(n_batches), desc="    [Mapping]"):
            start_idx = batch_idx * batch_size
            end_idx = min((batch_idx + 1) * batch_size, features.shape[0])
            
            batch_features = features[start_idx:end_idx]
            batch_labels = labels[start_idx:end_idx]
            bmu_indices, _ = self._find_bmu_batch(batch_features)
            
            for i, bmu_idx in enumerate(bmu_indices):
                bmu_idx = bmu_idx.item()
                label = batch_labels[i].item()
                if label not in neuron_votes[bmu_idx]:
                    neuron_votes[bmu_idx][label] = 0
                neuron_votes[bmu_idx][label] += 1
        
        for neuron_idx in range(self.n_neurons):
            if neuron_votes[neuron_idx]:
                votes = neuron_votes[neuron_idx]
                total_votes = sum(votes.values())
                if total_votes >= 1:
                    best_class = max(votes.items(), key=lambda x: x[1])[0]
                    confidence = votes[best_class] / total_votes
                    self.neuron_class_map[neuron_idx] = best_class
                    self.neuron_class_confidence[neuron_idx] = confidence
                else:
                    self._infer_from_neighbors(neuron_idx)
            else:
                self._infer_from_neighbors(neuron_idx)
        
        self._build_class_to_neurons_mapping()
        
        mapped = len([c for c in self.neuron_class_map.values() if c != -1])
        print(f"    Mapped: {mapped}/{self.n_neurons} neurons")
    
    def _build_class_to_neurons_mapping(self):
        self.class_to_neurons = defaultdict(list)
        for neuron_idx, cls in self.neuron_class_map.items():
            if cls != -1:
                self.class_to_neurons[cls].append(neuron_idx)
        for cls in self.class_to_neurons:
            self.class_to_neurons[cls] = np.array(self.class_to_neurons[cls])
    
    def _infer_from_neighbors(self, neuron_idx: int, k: int = 7):
        neuron_pos = self.positions[neuron_idx]
        distances = torch.norm(self.positions - neuron_pos, dim=1)
        distances[neuron_idx] = float('inf')
        _, nearest_indices = torch.topk(distances, k=min(k, self.n_neurons-1), largest=False)
        
        neighbor_votes = {}
        for neighbor_idx in nearest_indices:
            neighbor_idx = neighbor_idx.item()
            if neighbor_idx in self.neuron_class_map:
                neighbor_class = self.neuron_class_map[neighbor_idx]
                if neighbor_class not in neighbor_votes:
                    neighbor_votes[neighbor_class] = 0
                neighbor_votes[neighbor_class] += 1
        
        if neighbor_votes:
            best_class = max(neighbor_votes.items(), key=lambda x: x[1])[0]
            self.neuron_class_map[neuron_idx] = best_class
            self.neuron_class_confidence[neuron_idx] = 0.25
        else:
            self.neuron_class_map[neuron_idx] = -1
    ...
    def predict(self, features: torch.Tensor, batch_size: int = 256) -> np.ndarray:
        if self.class_to_neurons is None:
            raise RuntimeError("Must call assign_labels() before prediction!")
        ...
        for batch_idx in range(n_batches):
            ...
            for bmu_idx in bmu_indices:
                bmu_idx = bmu_idx.item()
                if bmu_idx in self.neuron_class_map:
                    predictions.append(self.neuron_class_map[bmu_idx])
                else:
...
class DualSOMSystem:
    ...
    def __init__(self, n_spiking_som: int = 400, n_lsm: int = 450,
                 n_traditional_som: int = 500, device: str = 'cpu',
                 time_steps: int = 50, confidence_threshold: float = 0.5,
        ...
        print("=" * 80)
        print("Dual SOM System Initialization (Feature Normalization Optimized Version)")
        print("=" * 80)
        
        print("\n  Step 1: Initializing encoder")
        self.encoder = EnhancedHybridSpikeEncoder(time_steps=time_steps, device=device)
        
        print("\n  Step 2: Initializing Spiking SOM (coarse clustering layer)")
        self.spiking_som = SpikingSOMLayer(n_input=784, n_competitive=n_spiking_som, device=device)
        
        print("\n  Step 3: Initializing LSM-STDP (deep feature extraction layer)")
        self.liquid_layer = OptimizedTemporalLiquidLayer(
            n_input=784, n_neurons=n_lsm, device=device,
            time_steps=time_steps, n_time_windows=5
        )
        
        self.lsm_feature_dim = 4 * n_lsm
        self.memory_feature_dim = 784
        
        print(f"\n  Feature Dimension Information:")
        print(f"    Memory vector dimension (simple sample original): {self.memory_feature_dim}")
        print(f"    LSM output feature dimension (hard samples): {self.lsm_feature_dim}")
        
        self.random_projector = RandomProjector(
            input_dim=self.memory_feature_dim,
            output_dim=self.lsm_feature_dim,
            device=device
        )
        print(f"    Random projection unified dimension: {self.lsm_feature_dim}")
        ...
        print(f"\n  Dual SOM system initialization completed")
        print(f"    Confidence threshold: {confidence_threshold}")
        print(f"    Feature normalization: {normalize_features}")
    
    def _normalize_features_batch(self, features: torch.Tensor) -> torch.Tensor:
        return F.normalize(features, p=2, dim=1)
    ...
    def process_batch(self, images: torch.Tensor, training: bool = True, 
        ...
        spike_train, encoding_info = self.encoder.encode(images)
        ...
        for b in range(batch_size):
            sample_spikes = spike_train[b]
            label = labels[b].item() if labels is not None else None
            bmu_idx, confidence, som_output, memory_vector = self.spiking_som.process_ttfs_spikes(
                sample_spikes, training=training, label=label
            )
            ...
            memory_vectors_list.append(memory_vector)
            
            if confidence >= self.confidence_threshold:
                easy_indices.append(b)
            else:
                hard_indices.append(b)
        
        easy_count = len(easy_indices)
        hard_count = len(hard_indices)
        self.processing_stats['easy_samples_count'] += easy_count
        self.processing_stats['hard_samples_count'] += hard_count
        self.processing_stats['total_samples'] += batch_size
        self.processing_stats['confidence_scores'].extend(confidence_scores)
        
        final_features = torch.zeros(batch_size, self.lsm_feature_dim, device=self.device)
        lsm_diagnostics = None
        
        if easy_count > 0:
            easy_indices_tensor = torch.tensor(easy_indices, dtype=torch.long, device=self.device)
            easy_memory_vectors = torch.stack([memory_vectors_list[i] for i in easy_indices])
            easy_proj = self.random_projector.project(easy_memory_vectors)
            
            if self.normalize_features:
                easy_proj = self._normalize_features_batch(easy_proj)
            
            final_features[easy_indices_tensor] = easy_proj
        
        if hard_count > 0:
            hard_indices_tensor = torch.tensor(hard_indices, dtype=torch.long, device=self.device)
            hard_spikes = spike_train[hard_indices_tensor]
            hard_features, lsm_diagnostics = self.liquid_layer.process(
                hard_spikes, self.time_steps,
                apply_stdp=training, training=training
            )
            
            if self.normalize_features:
                hard_features = self._normalize_features_batch(hard_features)
            
            final_features[hard_indices_tensor] = hard_features
        ...
        info = {
            ...
            'lsm_diagnostics': lsm_diagnostics,
            'memory_vectors': memory_vectors_list,
            'spike_train': spike_train,
        }
        
        return final_features, info
    ...
    def get_processing_stats(self) -> Dict:
        ...
        return {
            'total_samples': total,
            'easy_samples': self.processing_stats['easy_samples_count'],
            'hard_samples': self.processing_stats['hard_samples_count'],
            'easy_ratio': self.processing_stats['easy_samples_count'] / total,
            'hard_ratio': self.processing_stats['hard_samples_count'] / total,
            'avg_confidence': np.mean(self.processing_stats['confidence_scores'])
        }
